...
def blankMaker(org_txt, forbiddenLst, blank_maximum, deletedWord):
    commentSign = False
    ...
    for txt_vocab in org_txt:
        ...
        if txt_vocab.lower() in deletedWord:
            blankedList.append(txt_vocab)
            continue
        if txt_vocab.lower() != txt_vocab and ProgSetting['ignoreCapital']:
            blankedList.append(txt_vocab)
            continue
...
def linkSearch(textIn):
    if re.search(r'https://www\.ted\.com/talks/',textIn):
        print('TED link detected! The page will be opened in a browser. Follow instructions in the program window.')
        return 'TED'
    elif re.search(textIn,r'http://www\.listeningexpress\.com/cnn10/'):
        return 'CNN'
    else:
        return 'None'
